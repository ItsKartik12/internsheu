    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur-sm shadow-xs">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-3.5 sm:px-6 lg:px-8 gap-2 sm:gap-4">
          ...
          <nav className="hidden items-center gap-1.5 sm:gap-2 md:flex whitespace-nowrap">
            <NavLink
              to="/"
              className={({ isActive }) =>
                `whitespace-nowrap rounded-lg px-2.5 sm:px-3 py-1.5 text-xs font-semibold transition ${
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0 whitespace-nowrap">
            {user ? (
              <Link
                ...
                className="whitespace-nowrap shrink-0 flex items-center gap-1.5 rounded-xl bg-slate-900 px-3 sm:px-4 py-2 text-xs font-bold text-white shadow-sm transition hover:bg-slate-800"
              >
                <span>Go to Workspace</span> <ArrowRight size={13} className="shrink-0" />
              </Link>
            ) : (
              <>
                <Link
                  to="/login"
                  className="whitespace-nowrap shrink-0 rounded-lg sm:rounded-xl border border-slate-200 bg-white px-2.5 sm:px-3.5 py-1.5 text-xs font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50"
                >
                  Sign In
                </Link>
                <Link
                  to="/login"
                  state={{ mode: 'register' }}
                  className="whitespace-nowrap shrink-0 rounded-lg sm:rounded-xl bg-teal-600 px-3 sm:px-3.5 py-1.5 text-xs font-semibold text-white shadow-sm transition hover:bg-teal-500"
                >
                  Register
                </Link>
              </>
